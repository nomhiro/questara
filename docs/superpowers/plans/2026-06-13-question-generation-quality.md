...
// gpt-5 系は temperature パラメータ非対応のため、呼び出し側で temperature を渡さないこと。
...
// カタログ取得失敗時のフォールバック（チャット可能な代表モデル）
...
```
  - `openai` — OpenAI SDK を GitHub Models API（`https://models.github.ai/inference`、モデル ID は `openai/gpt-5` 形式、認証はユーザーの GitHub アクセストークン）に向けて使用。問題生成（既定 `openai/gpt-5`、UI で変更可）・資格抽出・アドベンチャー生成（既定 `openai/gpt-5-mini`）。gpt-5 系は `temperature` 非対応のため指定しない。モデル一覧は `modelCatalogService` がカタログ API から動的取得
```
