...
| D3 | モデル一覧はカタログ API `GET https://models.github.ai/catalog/models` から動的取得し、ドメインページの UI で選択可能にする | モデルは随時更新されるため静的リストにしない。失敗時の静的フォールバックは **gpt-5 系のみ**（`openai/gpt-5` / `gpt-5-mini` / `gpt-5-nano`） |
| D4 | LLM 呼び出し規約は **gpt-5 系をベース**に統一: `temperature` 等のサンプリングパラメータは送らない（gpt-5 系は非対応）、トークン上限を指定する場合は `max_completion_tokens` を使う。gpt-4 系はサポート対象外とし、フォールバックにも含めない | API エラー回避。4 系互換の分岐を持たずコードを単純に保つ |
