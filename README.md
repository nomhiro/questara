...
docker compose up -d cosmos-emulator   # Cosmos DB エミュレータを起動（seed / dev の前に必須）
...
2. 「GitHub でログインして始める」→ GitHub 認可 → 自動的に `/my/certifications`（マイ資格）へ
3. 公開資格から学ぶ場合は `/free-mode`（資格一覧）、独自資格は `/my/certifications/new` で追加
...
- **`routes/`** — Express ルート（landing / auth / certifications / domains / quiz / plans / ranking / profile / api）
...
| `GET /my/certifications` | マイ資格（ログイン後のホーム。要認証） |
| `GET /free-mode` | 資格一覧（公開資格＋自作資格。要認証） |
...
├── services/        # ビジネスロジック
├── middleware/      # 認証・HUD
├── views/           # EJS テンプレート（landing, my-certifications, quiz, ...）
